...
- **Agent-agnostic** - `claude`, `codex`, `pi`, `copilot`, `droid`, or `acp:<target>` via `acpx`, with ordered fallbacks.
...
`make e2e-record` overwrites `internal/e2e/fixtures/` from the real `claude` and `codex` CLIs, spends real API quota, and should be reviewed before committing.
