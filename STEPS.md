...
- [ ] **Slice 6b.2 — Bitbucket and Azure DevOps command wrappers and auth checks.** Port the Bitbucket and Azure DevOps CLI command wrappers and auth checks in `NoMistakes.Scm`, following the slice-6b.1 wrapper pattern. Tests cover both providers' command/argument construction and auth-check paths.
