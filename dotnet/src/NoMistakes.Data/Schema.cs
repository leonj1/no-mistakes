...
/// <summary>
/// Schema DDL and additive migrations for the run database. Mirrors Go's
/// internal/db/schema.go verbatim so both ports read and write the same tables.
/// </summary>
...
";
    ...
    /// <summary>
    /// Additive schema changes applied to databases created before the
    /// referenced columns existed. Each ALTER is idempotent because the
    /// "duplicate column name" error it raises when the column already exists is
    /// tolerated, keeping migrations lockless without a version table.
    /// </summary>
